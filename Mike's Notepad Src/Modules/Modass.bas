...
Public Sub AssociateText()
    'Associate Professional NotePad to plain text
    ...
    SaveSettingString HKEY_CLASSES_ROOT, App.EXEName + ".TXT" + "\DefaultIcon", "", SysDir + "\shell32.dll,-152"
      SaveSettingString HKEY_CLASSES_ROOT, ".pnt", "", App.EXEName + ".PNT"
    SaveSettingString HKEY_CLASSES_ROOT, ".pnt\ShellNew", "", ""
    SaveSettingString HKEY_CLASSES_ROOT, ".pnt\ShellNew", "NullFile", ""
    SaveSettingString HKEY_CLASSES_ROOT, App.EXEName + ".pnt" + "\shell\open\command", "", EXpath
    SaveSettingString HKEY_CLASSES_ROOT, App.EXEName + ".PNT" + "\DefaultIcon", "", SysDir + "\shell32.dll,-152"
...
Public Sub AssociateNotepad()
    ...
    'DeleteKey HKEY_CLASSES_ROOT, App.EXEName + ".TXT", "", "Professional NotePad Document"

    SHChangeNotify SHCNE_ASSOCCHANGED, SHCNF_IDLIST, 0, 0
End Sub
Public Sub AssociatePNTText()
    'Associate Professional NotePad to plain text
    'Associate Professional NotePad to plain text
    Dim EXpath As String
    If Right(App.Path, 1) = "\" Then
    ...
    SaveSettingString HKEY_CLASSES_ROOT, ".pnt\ShellNew", "", ""
    SaveSettingString HKEY_CLASSES_ROOT, ".pnt\ShellNew", "NullFile", ""
    SaveSettingString HKEY_CLASSES_ROOT, App.EXEName + ".PNT", "", "Mike's Text Editor!"
    SaveSettingString HKEY_CLASSES_ROOT, App.EXEName + ".PNT" + "\shell\open\command", "", EXpath
    SaveSettingString HKEY_CLASSES_ROOT, App.EXEName + ".PNT" + "\DefaultIcon", "", SysDir + "\shell32.dll,-152"
    SHChangeNotify SHCNE_ASSOCCHANGED, SHCNF_IDLIST, 0, 0
End Sub
Public Sub AssociatenoPNTText()
    'Associate Professional NotePad to plain text
    'Associate Professional NotePad to plain text
    Dim EXpath As String
    If Right(App.Path, 1) = "\" Then
    ...
    'SaveSettingString HKEY_CLASSES_ROOT, "" \ ShellNew
    'SaveSettingString HKEY_CLASSES_ROOT, ".pnt\ShellNew", "NullFile", ""
   ' SaveSettingString HKEY_CLASSES_ROOT, App.EXEName + " ", "", "Mike's Text Editor!"
    'S'aveSettingString HKEY_CLASSES_ROOT, App.EXEName + " " + "\shell\open\command", "", EXpath
    'SaveSettingString HKEY_CLASSES_ROOT, App.EXEName + " " + "\DefaultIcon", "", SysDir + "\shell32.dll,-152"
    SHChangeNotify SHCNE_ASSOCCHANGED, SHCNF_IDLIST, 0, 0
End Sub
Public Function IsAssociatedText() As Boolean
    ...
    End If
    'Dim Answer As String
    Answer = GetSettingString(HKEY_CLASSES_ROOT, ".pnt", "")
    If UCase(Answer) = UCase(App.EXEName + ".PNT") Then
        IsAssociatedText = True
    Else
        IsAssociatedText = False
    End If
...
Public Sub AddSCviewer()
    'make Professional NotePad default Sourcecode viewer for IE
...
Public Sub AddShortCutSendTo()
    ...
    sShortcutPath = SpecialFolder(9) + "\Michael J. Hardy's Professional NotePad.lnk"
    If IsObject(sh) Then
        Set link = sh.CreateShortcut(sShortcutPath)
        If IsObject(link) Then
            link.Description = "Advanced Skinnable Better NotePad Product..."
